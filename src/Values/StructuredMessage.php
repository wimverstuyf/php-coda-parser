...
class StructuredMessage
{
	...
	public function __construct(string $value, TransactionCode $transactionCode)
	{
		...
		} elseif ($this->structuredMessageType == "127" && $transactionCode->getFamily() == "05") {
			$this->sepaDirectDebit = new SepaDirectDebit(
				formatDateString(getTrimmedData($this->structuredMessageFull, 0, 6)),
				getTrimmedData($this->structuredMessageFull, 6, 1),
				getTrimmedData($this->structuredMessageFull, 7, 1),
				getTrimmedData($this->structuredMessageFull, 8, 1),
				getTrimmedData($this->structuredMessageFull, 9, 35),
				getTrimmedData($this->structuredMessageFull, 44, 35),
				getTrimmedData($this->structuredMessageFull, 79, 62),
				getTrimmedData($this->structuredMessageFull, 141, 1),
				getTrimmedData($this->structuredMessageFull, 142, 4)
			);
		}
	}
